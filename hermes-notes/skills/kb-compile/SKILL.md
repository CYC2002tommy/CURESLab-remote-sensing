...
1. **Scan for Unprocessed Files:**
   Use `Grep` or `Bash` to identify markdown files in `raw/` and `artifacts/` that DO NOT have a corresponding summary in `wiki/summaries/`.
...
4. **Update Indexes:**
   - Append new sources to `wiki/indexes/All-Sources.md` using `Edit` or `Bash`.
