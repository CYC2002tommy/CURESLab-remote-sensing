...
- **SSH PTY vs. MFA/OTP**: 
  - *Symptom*: When logging into an HPC via `terminal(pty=true)` that requires Two-Factor Authentication (OTP), the agent's interactive process may crash with `argument 'to_write': 'bytes' object cannot be converted to 'PyString'` upon attempting to write auth responses.
...
- **Blind `sed` Replacements in Scientific Scripts**: 
  ...
  - *Fix*: Never assume the format of a variable declaration (e.g., assuming `-d0 YYYY MM DD` or `YYYYMMDD`). ALWAYS `cat` or `read_file` the target script first to inspect how the parameters are declared (e.g., `year=2025\nmonth=3\nday=24`). Use precise regexes anchored to the variable name (e.g., `sed -i -r "s/^year=[0-9]+/year=2021/g"`).
