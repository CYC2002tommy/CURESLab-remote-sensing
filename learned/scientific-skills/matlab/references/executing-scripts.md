...
### MATLAB: "No sign of mw-spf-ui after 15 seconds. Aborting..."

This error occurs when MATLAB fails to initialize its Service Platform Framework (UI layer), which is common in headless environments, CI pipelines, or when zombie `MATLAB.exe` processes are stuck in the background.

**Fixes:**
1. **Kill zombie processes:** Use Task Manager or `taskkill /F /IM MATLAB.exe` (Windows) / `killall MATLAB` (Unix) to clear out stuck background instances.
2. **Run strictly headless:** Ensure you are using `-nodesktop -nosplash`. Note that `-batch` is modern but might still trigger SPF; if it fails, fallback to `-nodesktop -nosplash -r "run('script.m'); exit;"`.
3. **Agent fallback:** If an AI agent encounters this while trying to verify math or execute a script via `terminal()`, it should **immediately fallback to Python (NumPy/SciPy)** or Octave rather than fighting the MATLAB UI framework.
