...
### Drafting Speaker Notes / Presentation Scripts

When a user asks you to draft a speech, presentation script, or speaker notes based on an existing slide deck (PDF or PPTX):
1. **Slide-by-Slide Mapping (CRITICAL)**: Always structure the script page-by-page (逐頁配稿), matching the exact pagination and sequence of the source presentation. Do not provide a generic thematic outline unless explicitly requested.
2. **Timing**: Include estimated time allocations per slide based on the overall requested duration (e.g., using the ~1 slide per minute rule).
3. **Tone**: Adopt a professional, spoken tone appropriate for the audience (e.g., academic defense, business pitch), removing meta-commentary.
4. **Integration**: Weave in findings from accompanying detailed reports (e.g., DOCX/PDF manuscripts) to add depth to the slide's visual bullet points.
