...
**Pitfall (400 Bad Request / Invalid user ID):** The `library_id` parameter MUST be the numeric integer ID (e.g., `16500033`), NOT the string username (e.g., `tommydedelanda`). If the user provides a string username and you receive a `400 Bad Request` with `Invalid user ID`, use the `https://api.zotero.org/keys/{api_key}` endpoint to programmatically discover the correct numeric `userID`.
...
| File | Contents |
|------|----------|
| [templates/crossref_to_zotero.py](templates/crossref_to_zotero.py) | Full example script: fetches authoritative metadata from Crossref API, creates a parent item, and attaches a local PDF |
