...
- You are debugging `execute_code` or `terminal` scripts that fail due to missing dependencies.
...
- **Do:** Use the `terminal` tool to invoke the script using the absolute path to the Python executable of the fully-configured environment.
  ```bash
