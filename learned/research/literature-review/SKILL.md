...
- Execute the script using `python3 {script.py}` (avoid `execute_code` due to potential missing `python-docx` in the internal sandbox; use standard `terminal` tool).
...
- **Missing `python-docx`**: Do not use `execute_code` for `python-docx` if it throws `ModuleNotFoundError`. Instead, write a python script to `/tmp` via heredoc (`cat << 'EOF' > /tmp/script.py`) and run it with `python3` in the `terminal` tool.
- **Background Processes**: Do not use `&` for backgrounding processes in standard foreground `terminal` calls.
