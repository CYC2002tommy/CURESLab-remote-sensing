...
2. **Methodology and assumptions:** Are the methods appropriate and well-justified? Identify any hidden assumptions or unrealistic simplifications. Are there methodological gaps, missing controls, or biases? Is the data sufficient and reliable? **Quantitative Verification (CRITICAL):** Do not take mathematical matrices, equations, or table summaries at face value. Use your tools (`pdftotext -layout`, `grep`, or `execute_code`) to extract raw data and verify constraints (e.g., check if an I-O matrix is actually invertible via determinant/spectral radius, recalculate column sums, check unit consistency). Flag mathematically impossible claims as critical flaws.
