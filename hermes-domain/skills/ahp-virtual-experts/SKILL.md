...
4. **Mathematical Verification:** Python scripts (`execute_code`) are used to compute the Principal Eigenvalue ($\lambda_{max}$), Consistency Index (CI), and Consistency Ratio (CR) to eliminate hallucinated or illogical weightings.
...
1. **Write the Verifier Script:** Use the `execute_code` tool to write a Python script (using `numpy`) that inputs the aggregated matrix.
2. **Calculate:** 
...
- **Never guess the math:** Large language models are bad at matrix algebra. **Always** use `execute_code` with `numpy` to calculate eigenvectors and Consistency Ratios.
